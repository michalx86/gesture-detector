...
def main():
  ...
  for category, image_list in test_set.items():
    ...
    for img_name in image_list:
      ...
      if len(candidates) == 0 :
        img_no_answer = img_no_answer + 1
        category_img_no_answer = category_img_no_answer + 1
      for i in range(top_k):
        if i < len(candidates):
          print("     {} {:.0%}".format(labels_map[candidates[i].id], candidates[i].score))
          if labels_map[candidates[i].id] == category:
            correct[i] = correct[i] + 1
            if i == 0 :
              category_correct = category_correct + 1
          else:
            if i == 0 :
              category_wrong = category_wrong + 1
            wrong[i] = wrong[i] + 1
    category_correct_and_wrong = category_correct + category_wrong
    print('1-st Choice Correct: {}/{} - {:.0%}'.format(category_correct, category_correct_and_wrong , category_correct/category_correct_and_wrong if category_correct_and_wrong > 0 else 0.0)) 
    print('No-clue images: {}/{} - {:.0%}'.format(category_img_no_answer, len(image_list), category_img_no_answer/len(image_list)))
  print('----------------     Evaluation result   -----------------')
  for i in range(top_k):
    correct_and_wrong = correct[i] + wrong[i]
    print('Top {} : {:.0%}'.format(i + 1, correct[i] / correct_and_wrong if correct_and_wrong > 0 else 0.0))
  print('Total no-clue images: {} / {} - {:.0%}'.format(img_no_answer, img_num, img_no_answer / img_num))
